...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  const { syncStatus, syncError, contactMessages, siteSettings } = useContent();
  const navigate = useNavigate();
  const location = useLocation();
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const unreadMessages = contactMessages.filter((m) => !m.read).length;
  const isLive = !!(siteSettings.liveEvent && siteSettings.liveEvent.isLive);
  ...
  const navGroups: NavGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { path: '/admin/live', label: 'Live Stream', icon: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', badge: isLive ? 'LIVE' : undefined },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { path: '/admin/messages', label: 'Inbox Messages', icon: 'M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.253-.94L3 20l1.1-3.418A7.992 7.992 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z', badge: unreadMessages > 0 ? `${unreadMessages}` : undefined },
      ],
    },
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col md:flex-row antialiased font-sans">
      ...
      >
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-6">
          ...
          <div className="flex items-center justify-between px-2 pb-2 border-b border-slate-100 dark:border-slate-800">
            <Link to="/admin/dashboard" className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center font-black text-sm font-display shadow-sm">
                D
              </div>
          ...
          <nav className="space-y-5">
            {navGroups.map((group) => (
              <div key={group.groupTitle} className="space-y-0.5">
                <div className="px-2 mb-2 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-2">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Badge for unread messages or LIVE status */}
                      {item.badge && (
                        <span className={`ml-auto flex-shrink-0 px-1.5 py-0.5 rounded-full text-[10px] font-black ${
                          item.badge === 'LIVE'
        ...
        <div className="p-3 border-t border-slate-200/80 dark:border-slate-800 space-y-1">
          <Link
            to="/"
            target="_blank"
            className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {syncStatus === 'error' && syncError && (
          <div className="bg-red-50 dark:bg-red-950/40 border-b border-red-200 dark:border-red-900 px-6 py-2.5 flex items-center gap-2 text-xs text-red-700 dark:text-red-400">
